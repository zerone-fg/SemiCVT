...
from semicvt.utils.loss_helper import get_criterion
...
def train(
    ...
    for step in range(len(data_loader)):
        ...
        outs_cnn, outs_trans = model(image)
        pred_cnn = outs_cnn["pred"]
        pred_cnn = F.interpolate(pred_cnn, (h, w), mode="bilinear", align_corners=True)
        pred_trans = outs_trans["pred"]
        pred_trans = F.interpolate(pred_trans, (h, w), mode="bilinear", align_corners=True)

        if "aux_loss" in cfg["net"].keys():
            aux_cnn = outs_cnn["aux"]
            aux_cnn = F.interpolate(aux_cnn, (h, w), mode="bilinear", align_corners=True)
            loss_cnn = criterion([pred_cnn, aux_cnn], label)
            aux_trans = outs_cnn["aux"]
            aux_trans = F.interpolate(aux_trans, (h, w), mode="bilinear", align_corners=True)
            loss_trans = criterion([pred_trans, aux_trans], label)
        else:
            loss_cnn = criterion(pred_cnn, label)
            loss_trans = criterion(pred_trans, label)

        loss = 0.5 * loss_cnn + 0.5 * loss_trans
